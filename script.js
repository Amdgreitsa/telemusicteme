(async () => {
    async function detectChromasyncUltimate() {
        try {
            const tries = 10;
            const delay = (ms) => new Promise((r) => setTimeout(r, ms));
            for (let i = 0; i < tries; i++) {
                if (typeof window !== 'undefined') {
                    const result = await window.ENABLED_ADDONS();
                    const addonIds = Array.isArray(result?.theme) ? result.theme.map((a) => String(a?.name || '').toLowerCase()) : [];
                    return addonIds.includes('chromasyncultimate') || addonIds.includes('chromasynclite');
                }
                await delay(100);
            }
        } catch (e) {
    ...
    const isChromaSync = await detectChromasyncUltimate();

    if (isChromaSync) return;
    ...
    function setPlayerBackground(settings, image) {
        ...
        if (blur.value !== undefined) {
            backgroundDiv.style.setProperty('--blur', `${blur.value}px`);
        } else {
            backgroundDiv.style.setProperty('--blur', `${blur.default}px`);
        }
    }
    ...
    function rememberContext(settings, styles, state) {
        if (settings) lastSettings = settings;
        if (styles) lastStyles = styles;
        if (state) lastState = state;
    }

    function forceRefreshFromLastContext() {
        if (!lastSettings || !lastState) return;

        const playerBackground = lastSettings.get('playerBackground');
        const backgroundImage = lastSettings.get('backgroundImage');

        if (!playerBackground) return;

        const image = playerBackground.value
            ? 'https://' + (lastState.track?.coverUri || '').replace('%%', '1000x1000')
            : (backgroundImage?.value || '').replace(/\\/g, '/');

        if (image) {
            setPlayerBackground(lastSettings, image);
        }

        if (lastStyles && lastSettings.get('betterPlayer')?.value) {
            setupBetterPlayer(lastSettings, lastStyles);
        }

        observeSyncLyricsChanges();
        scheduleRefresh();
    }

    function findNativeSyncLyricsButton() {
        return document.querySelector('.FullscreenPlayerDesktopControls_syncLyricsButton__g6E6g[data-test-id="PLAYERBAR_DESKTOP_SYNC_LYRICS_BUTTON"]');
    }

    function isNativeSyncEnabled(btn) {
        if (!btn) return false;
    ...
    function scheduleRefresh() {
        ...
        setTimeout(() => {
            ...
            refreshCustomSyncState();
        }, 50);
    }

    function observeSyncLyricsChanges() {
        if (nativeBtnObserver) {
            nativeBtnObserver.disconnect();
            nativeBtnObserver = null;
        }
        if (syncRetryTimer) {
        ...
        const nativeBtn = findNativeSyncLyricsButton();
        if (!nativeBtn) {
            syncRetryTimer = setInterval(() => {
                const btn = findNativeSyncLyricsButton();
                if (btn) {
                    clearInterval(syncRetryTimer);
                    syncRetryTimer = null;
                    observeSyncLyricsChanges();
        ...
        nativeBtnObserver = new MutationObserver(() => {
            scheduleRefresh();
        });

        nativeBtnObserver.observe(nativeBtn, {
            attributes: true,
    ...
    function disconnectSyncObserver() {
        if (nativeBtnObserver) {
            nativeBtnObserver.disconnect();
            nativeBtnObserver = null;
        }
    ...
    function refreshCustomSyncState() {
        const customBtn = document.querySelector('.custom-text');
        const nativeBtn = findNativeSyncLyricsButton();
        if (!customBtn) return;

        if (nativeBtn) {
            const enabled = isNativeSyncEnabled(nativeBtn);
            customBtn.disabled = !enabled;

            customBtn.onclick = null;
            if (enabled) {
                customBtn.onclick = () => nativeBtn.click();
            }

            const customIcon = customBtn.querySelector('svg');
            const nativeIcon = nativeBtn.querySelector('svg');
            if (customIcon && nativeIcon) {
                const active = nativeIcon.classList.contains('SyncLyricsButton_icon_active__6WcWG');
                if (active) {
                    customIcon.classList.add('SyncLyricsButton_icon_active__6WcWG');
                } else {
                    customIcon.classList.remove('SyncLyricsButton_icon_active__6WcWG');
                }
            }
        } else {
            customBtn.disabled = true;
            const customIcon = customBtn.querySelector('svg');
            if (customIcon) customIcon.classList.remove('SyncLyricsButton_icon_active__6WcWG');
            customBtn.onclick = null;
        }
    }
    ...
    function setupBetterPlayer(settings, styles) {
        ...
        const syncLyricsButton = findNativeSyncLyricsButton();
        ...
        if (!customControls) {
            ...
            if (playQueueButton) customControls.appendChild(playQueueButton);
            if (contextMenu) customControls.appendChild(contextMenu);
            customControls.appendChild(controls);
            if (likeButton) customControls.appendChild(likeButton);
            ...
            newCustomLyricsButton.innerHTML =
                '<span class="JjlbHZ4FaP9EAcR_1DxF"><svg class="J9wTKytjOWG73QMoN5WP o_v2ds2BaqtzAsRuCVjw"><use xlink:href="/icons/sprite.svg#syncLyrics_m"></use></svg></span>';

            if (syncLyricsButton) {
                clickCustomText(syncLyricsButton, newCustomLyricsButton);
                const isActive = syncLyricsButton.querySelector('svg')?.classList.contains('SyncLyricsButton_icon_active__6WcWG');
                if (isActive) {
            ...
            styles.add(
                ...
                .SonataFullscreenControlsDesktop_sonataButton__69FFc:disabled,
                ...
                .SonataFullscreenControlsDesktop_sonataButton__69FFc:not(:disabled):focus-visible,
                .SonataFullscreenControlsDesktop_sonataButton__69FFc:not(:disabled):hover,
                ...
                .SonataFullscreenControlsDesktop_sonataButton__69FFc[data-test-id="PLAY_BUTTON"],
                .SonataFullscreenControlsDesktop_sonataButton__69FFc[data-test-id="PAUSE_BUTTON"] {
                    border-style: solid;
                    border-width: 3px;
                ...
                .SonataFullscreenControlsDesktop_root__ZCIGk {
                    gap: 0;
                }
    ...
    betterPlayer.player.on('trackChange', ({ settings, state }) => {
        rememberContext(settings, null, state);
    ...
    betterPlayer.player.on('openText', () => {
    ...
    betterPlayer.player.on('closeText', () => {
        const customLyricsButton = document.querySelector('.custom-text');
    ...
    betterPlayer.settingsManager.on('change:backgroundImage', ({ settings }) => {
        rememberContext(settings, null, null);
    ...
    betterPlayer.settingsManager.on('change:betterPlayer', ({ settings, styles }) => {
        rememberContext(settings, styles, null);
    ...
    betterPlayer.settingsManager.on('change:playerButtonsBackground', ({ settings, styles }) => {
        ...
        const betterPlayerSetting = settings.get('betterPlayer');
        if (!betterPlayerSetting) return;
        if (!betterPlayerSetting.value) return;
